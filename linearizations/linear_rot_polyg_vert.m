function V = linear_rot_polyg_vert(s_coo, m_coo, s_nod, m_nod, x0, n0, N0, C0, ...
...
for v=1:n_vert
  ...
  if clip_origin(v, 1) == 1
    P = linear_project_onto_plane(s_coo(:,clip_origin(v,2)), s_nod(clip_origin(v,2)), x0, n0, N0, C0, 1, nN_m);  
  ...
  elseif clip_origin(v, 1) == 2
    P = linear_project_onto_plane(m_coo(:,clip_origin(v,2)), m_nod(clip_origin(v,2)), x0, n0, N0, C0, 2, nN_m);  
  ...
  elseif clip_origin(v, 1) == 3
    ...
    P_A = linear_project_onto_plane(s_coo(:,clip_origin(v,2)), s_nod(clip_origin(v,2)), x0, n0, N0, C0, 1, nN_m);
    P_B = linear_project_onto_plane(s_coo(:,clip_origin(v,3)), s_nod(clip_origin(v,3)), x0, n0, N0, C0, 1, nN_m);
    P_C = linear_project_onto_plane(m_coo(:,clip_origin(v,4)), m_nod(clip_origin(v,4)), x0, n0, N0, C0, 2, nN_m);
    P_D = linear_project_onto_plane(m_coo(:,clip_origin(v,5)), m_nod(clip_origin(v,5)), x0, n0, N0, C0, 2, nN_m);
