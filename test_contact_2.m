...
for n = 1:timestepping.steps
  ...
  for c = 1:1
    ...
    while newton_continue
      ...
      AA = [ DK B_' Z1; B_ zeros(size(B_,1),size(B_,1)+size(Z1,2)); [tmp_normals*Z1'; tmp_tangen1*Z1'; tmp_tangen2*Z1'] zeros(size(Z1,2),size(B_,1)+size(Z1,2))];
      bb = [-r; c_; weight_gap; 0*weight_gap; 0*weight_gap];
      result = AA\bb;
