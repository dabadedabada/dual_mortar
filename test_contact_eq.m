...
mesh{1} = mesh_generator([7,7,2], [6,6,2],etype{1});
mesh{2} = mesh_generator([9,9,1], [2,2,2],etype{2});
...
if false
  mesh{1}.nod.coo = mesh{1}.nod.coo + [0,0,1.001];
else
  tmp = 4;
...
consts = cell(2,1);
consts{1} = struct('Young', 1e5, 'Poisson', 0.3, 'density', 1e-1, 'lambda', [], 'mu', []);
consts{2} = struct('Young', 2e6, 'Poisson', 0.4, 'density', 2e-1, 'lambda', [], 'mu', []);
...
problem{1}.boundary.dirichlet = {...
  6,'0*[X,Y,Z]+[ 1.5,0,0]*max(T-0.2,0)+[0,0,-1.0]*min(T,0.2)','enforce'};
...
for n = 1:timestepping.steps
  ...
  for c = 1:1
    ...
    tmp_tangen1 = mat2cell(normals_storage.t1              ,ones(size(normals_storage.t1              ,1),1),3);
    tmp_tangen2 = mat2cell(normals_storage.t2              ,ones(size(normals_storage.t2              ,1),1),3);
    tmp_normals = blkdiag(tmp_normals{:});
    tmp_tangen1 = blkdiag(tmp_tangen1{:});
    tmp_tangen2 = blkdiag(tmp_tangen2{:});
    % mort_M = mort_D\mort_M;
    % mort_D = mort_D\mort_D;
    ND = tmp_normals*mort_D; tmpmap_s = reshape([3*cont_face{1}.map.nod-2 3*cont_face{1}.map.nod-1 3*cont_face{1}.map.nod]',[],1); [i_,j_,v_] = find(ND); ND = sparse(i_,tmpmap_s(j_),v_,max(i_),mesh{1}.info.noddof*mesh{1}.info.nodcount);
    NM = tmp_normals*mort_M; tmpmap_m = reshape([3*cont_face{2}.map.nod-2 3*cont_face{2}.map.nod-1 3*cont_face{2}.map.nod]',[],1); [i_,j_,v_] = find(NM); NM = sparse(i_,tmpmap_m(j_),v_,max(i_),mesh{2}.info.noddof*mesh{2}.info.nodcount);
    gap = -[ND, -NM]*[X{1}; X{2}];
    ...
    while newton_continue
      ...
      f_c = zeros(3*(mesh{1}.info.nodcount + mesh{2}.info.nodcount),1);   f_c(idx_s_glob) = mort_D*z; f_c(idx_m_glob) = -mort_M'*z;
      ...
      %bb = [-r; c_ ; zeros(size(row5,1),1); -weight_gap(activ_loc); zeros(size(row7,1),1)];
      AA = [ DK_ Z1; [tmp_normals*Z1'; tmp_tangen1*Z1'; tmp_tangen2*Z1'] zeros(size(Z1,2))];
      bb = [-r_; weight_gap; 0*weight_gap; 0*weight_gap];
      %AA = DK_;
      %bb = -r_;
      result = AA\bb;
      ...
      %figure(3); patch('Faces',cont_face{1}.nod,'Vertices',cont_face{1}.coo,'FaceColor','red')
      ...
      %figure(3); patch('Faces',cont_face{1}.nod,'Vertices',cont_face{1}.coo,'FaceColor','red')
      %figure(3); patch('Faces',cont_face{2}.nod,'Vertices',cont_face{2}.coo,'FaceColor','red')
